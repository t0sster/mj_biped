...
```
команд принято: 100/с, фидбек моторов: 1000/с, пакетов IMU: 600/с, кадров с шины: 1000/с, ошибок CAN: 0/с, шина: ACTIVE
```
...
Если фидбека нет, «кадров с шины» отвечает, кто виноват:

- **кадров 0** — на шине действительно тишина: моторы не запитаны, не тот
  битрейт или can0 не поднят;
- **кадры идут, фидбек 0** — моторы отвечают, но не тем Frame ID, который мы
  считаем своим. Нода печатает список увиденных ID; нужный вписывается
  в `CAN_MASTER_ID` (`damiao_can.py`) или меняется в настройках моторов.
...
конкретная железка. **Ноду при этом надо остановить:** порт IMU открывается
монопольно, иначе два читателя делили бы байты между собой и данные выглядели
бы как шум. Второй процесс теперь получает явную ошибку.
