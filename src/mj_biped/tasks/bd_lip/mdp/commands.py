...
>>>>>>> dev-biped-pp-alt
    ...
    if self.cfg.sync_velocity_command:
      step_time = self._step_time(gait_command)
      cmd[:, 0:1] = self.step_length / step_time
      cmd[:, 1] = 0.0
    cmd_vel_b = cmd[:, :2]
    cmd_wz = cmd[:, 2:3]
    ...
    if swing_start[:, 0].any():
      ...
      self.step_targets_w[right_ids, 0, :2] = plan.target_xy_w[right_ids]
      self.step_targets_w[right_ids, 0, 2] = plan.target_heading_w[right_ids, 0]
...
<<<<<<< HEAD
  ...
  def _step_time(self, gait_command: torch.Tensor) -> torch.Tensor:
    if torch.any(self.step_period.abs() > 1.0e-8):
      return self.step_period
    if self.cfg.step_period_s is not None:
      return torch.full((self.num_envs, 1), self.cfg.step_period_s, device=self.device)
    freq = gait_command[:, 0].clamp(min=1.0e-3)
    return (0.5 / freq).unsqueeze(1)
