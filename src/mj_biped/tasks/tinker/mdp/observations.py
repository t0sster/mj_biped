...
def imu_rpy(env, asset_cfg: SceneEntityCfg) -> torch.Tensor:
  ...
  No sign/axis correction here -- the imu site's quat in tinker_range.xml
  already encodes the chip's real physical mounting (verified against a
  static real accelerometer reading), so the raw euler_xyz_from_quat output
  lines up with the real chip's convention as-is.
  """
  asset = env.scene[asset_cfg.name]
